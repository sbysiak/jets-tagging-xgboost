...
start_0 = time()
df_orig = ahelp.read_jet_extractor(fnames='../../../benchmark-dataset/DATA/trains/train_LHC17f8g_11.root', n_rows=10000)
df = df_orig.copy()
df_full = ahelp.unroll_df(df, n_constit_sv={'fConstituents':3, 'fSecondaryVertices':3}, separate=False)
df = df_full.copy()
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#!!!  PROVIDE DESCRIPTION OF EXPERIMENT  &  PROJECT NAME !!!!
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
experiment = Experiment(api_key="Vr3JAwshw0tYqpsQluIhQxdaL",
                        project_name='no-proj-name-provided')
experiment.log_other('descr', 'no descr.')
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
experiment.log_other('t_data_preparing [sec]', round(time_data_preparing,1))
...
param_dist =  {
                ...
                'colsample_bytree':[round(x,2) for x in  np.arange(0.3, 1.01, 0.1)],
                'colsample_bylevel':[round(x,2) for x in np.arange(0.3, 1.01, 0.1)],
                'subsample':[round(x,2) for x in np.arange(0.3, 1.01, 0.1)],
                'gamma':[round(x,6) for x in np.logspace(-5,3,17)]+[0]*5
                }


start = time()
clf_rand = RandomizedSearchCV(xgb.XGBClassifier(), param_distributions=param_dist, n_iter=50,
                              scoring='roc_auc', cv=5, verbose=1, return_train_score=True)
...
experiment.log_other('t_model_training [sec]', round(time_training,1))
...
with experiment.validate():
    experiment.log_metric('roc_auc', max(cv_res['mean_test_score']))

...
void generate(const char *module = "__ALL__")
{
  ...
  TString periodName(gSystem->Getenv("PERIOD_NAME"));//PERIOD_NAME
   ...
   //plugin->SetInputFormat("xml-single");debugLevel
   plugin->SetAnalysisMacro(Form("%s.C", train_name));
   ...
   if (dataBasePath.Length() > 0)
   {
     ...
     TString dataTag;
     dataTag.Form("%s_%s_%s", dataBasePath.Data(), archiveName.Data(), dataAnchor.Data());
   ...
   if (atoi(gSystem->Getenv("AOD")) == 100) { // MC production
     ...
     Printf("totalEvents, splitMaxInputFileNumber = %lld %d", totalEvents, splitMaxInputFileNumber);
